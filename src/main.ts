...
import {
  factor7, REF, solveCommas, optimizeCommas, ordSuffix, octStr, compoundOrd,
  fmtInterval, intervalName, shortenInterval, equalIntervalName, letterIdx,
} from './tuning/intervals.js';
import { chordTemplates, invNames, isPow2, lcm, analyzeChord } from './tuning/chords.js';
...
function updateInfo(){
  ...
  if(keys.length>=2){
    ...
    var prevNum=-1;
    var shortIvl=document.getElementById('cbShortIvl').checked;
    for(var idx=0;idx<ivls.length;idx++){
      ...
      if(equalEnabled){
        ...
        var iname=shortenInterval(equalIntervalName(keys[iv.i].q,keys[iv.i].r,keys[iv.j].q,keys[iv.j].r),shortIvl);
        ...
        var iname=shortenInterval(intervalName(iv.rat.num,iv.rat.den,iv.rat.e),shortIvl);
