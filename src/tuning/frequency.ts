...
import { equalEnabled, septimalEnabled, septimalShift } from '../state/tuning.js';

export function keyFreq(q, r) {
  if (equalEnabled) return 220 * Math.pow(2, (4 * q + 7 * r) / 12);
  ...
  if (septimalEnabled) {
    ...
    if (septimalShift !== 0) f *= Math.pow(80 / 81, septimalShift / 6);
  }
  return f;
}
