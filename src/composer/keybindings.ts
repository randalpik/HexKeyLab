...
export const KEYBINDINGS: KeySection[] = [
  ...
  {
    title: 'Voice mode',
    intro: 'Active when the current voice is 1..4. Step entry follows Finale convention.',
    bindings: [
      {
        keys: '1 .. 7',
      ...
      { keys: '.', desc: 'Cycle dots on current note/chord/rest (0 → 1 → 2 → 0).' },
      { keys: '=', desc: 'Toggle tie on current note (or selected chord note).' },
      {
      ...
      { keys: '↑  /  ↓', desc: 'Previous / next voice. Cycle: 1 → 2 → expr → 3 → 4.' },
      { keys: '←  /  →', desc: 'Move cursor left / right within the current voice.' },
      { keys: 'Home  /  End', desc: 'Jump to start / end of the current voice.' },
      ...
      { keys: 'Ctrl+←  /  Ctrl+→', desc: 'Jump to first cursor stop of previous / next measure.' },
      {
        keys: 'Alt+↑  /  Alt+↓',
  ...
  },
  {
    ...
    intro: 'Voice-mode sub-flow. Tuplets are non-nested and live within a single measure.',
    bindings: [
      {
        keys: 'Ctrl+2 .. Ctrl+7',
      ...
      { keys: 'Escape', desc: 'Cancel a pending tuplet (between Ctrl+N and the digit).' },
      {
  ...
  },
  {
    ...
    intro: 'A virtual voice between voices 2 and 3 in the cycle. Edits expression marks (dynamics, hairpins).',
    bindings: [
      { keys: '←  /  →', desc: 'Step through the unified moment list (note onsets across all voices ∪ existing expression moments).' },
      {
        keys: '1 .. 8',
      ...
      { keys: '<  /  >', desc: 'Hairpin mark-start / mark-end (same two-step flow as voice mode).' },
      { keys: 'Backspace  /  Delete', desc: 'Delete the selected expression element at this moment (dynam first, then any containing hairpin).' },
      ...
      { keys: '↓', desc: 'Leave expression mode downward (forward to voice 3).' },
    ],
  },
  {
    title: 'Selection mode',
    intro: 'Entered from voice mode via Shift+arrow. Selects a contiguous range of beats (one voice) or measures (one or more two-voice staves).',
    bindings: [
      { keys: 'Shift+←  /  Shift+→', desc: 'Enter beat selection (from voice mode), or extend the selection by one beat (in selection mode).' },
      {
        keys: 'Shift+↑  /  Shift+↓',
      ...
      { keys: 'Ctrl+Shift+←  /  Ctrl+Shift+→', desc: 'Extend the beat selection by a whole measure at a time.' },
      ...
      { keys: 'Backspace  /  Delete', desc: 'Delete the selection content and exit to voice mode (no clipboard write).' },
      { keys: 'Escape', desc: 'Exit selection mode (cursor returns to the movable end of the range).' },
    ],
  },
];
