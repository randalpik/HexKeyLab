...
**HKL Composer** (`apps/composer/`) is a sibling app shipped from the same repo — a keyboard-driven, Verovio-backed score editor that uses HKL as its input device via a same-origin `BroadcastChannel` bridge. Composer holds the MEI/score state; HKL holds the audio/MIDI/tuning state. See architecture.md §7.
...
- **Analyzer-side code** is the self-contained `apps/analyzer/` package (entry `apps/analyzer/index.html`). Layout: `src/` (browser UI), `analysis/` (the DOM-free signal-processing engine modules `analyzer-analysis.js`, `analyzer-instruments.js`, `k-weighting.js` + the Canvas-using `analyzer-visualization.js` — shared by the UI *and* the CLI), `cli/` (the Node batch pipeline), `configs/*.json` (instrument source-of-truth). It may import `@hkl/shared`, `@hkl/engine`, `@hkl/bridge` and its own in-package `analysis/`; it must NOT import another app's `src/` (`apps/hkl`, `apps/composer`). Heavy signal-processing runs in `apps/analyzer/src/pipeline-worker.ts` (Vite `?worker`); main-thread `decodeAudioData` ships per-channel Float32Arrays to the worker as transferables. **CLI workflow** (`pnpm analyze` → `apps/analyzer/cli/generate-samples.js`, reads `apps/analyzer/configs/*.json`, then `insert-instrument.js` splices the block into `apps/hkl/src/audio/samples-data.ts`) is the source of truth for shipped instruments. The old single-file `HexKeyLab-analyzer.html` sidecar was deleted in the cleanup (see decisions.md).
