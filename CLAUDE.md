...
- **`apps/{hkl,composer,analyzer,orchestrator}/`** — the four servable apps, each its own package with `index.html` + `vite.config.ts` + `package.json` + `src/`. (Granular paths in this doc written as `src/foo/bar.ts` now live at `apps/hkl/src/foo/bar.ts` unless they're composer/analyzer/orchestrator code.) Each runs its own dev server with scoped HMR. (`orchestrator` = HKLO: samples a MIDI device into a velocity-layered `.hki` — see [docs/architecture/orchestrator.md](docs/architecture/orchestrator.md).)
