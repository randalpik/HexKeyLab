...
- **Power-off emits a pitch bend before it emits garbage notes.** `readWheelADC` polls the wheel over I2C every main-loop pass; a keystroke first needs a PIC GPIO + CTS handshake. So the collapsing rail produces a degenerate bend strictly *before* any spurious keystroke — which is why HKL treats inbound 0xE0 as a disconnection (decisions.md, 2026-09-21).
