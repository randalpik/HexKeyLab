...
- **`'V'`** Schismatic *(experimental study mode)* — same qm shifts as Pythagorean, but `freqAt` multiplies the band factor by `SCHISMA^b` (where schisma = 32805:32768 ≈ 1.954c is the comma by which 2·PM3 + M3 exceeds an octave). Result: every band is `(PM3, pure 5/4, PM3)` summing to octave + schisma. The seam-crossing third — Pythagorean's tempered dim4 — becomes a pure 5/4, at the cost of octave drift accumulating to ~14c across 7 octaves. Magnitude lands in the Railsback central-stretch range (~1–3c/oct), so pianistically-trained ears find it natural rather than jarring; V isolates the pure-thirds component of natural octave stretch from the inharmonicity component pianos add on top. Two non-trivial dispatches diverge from Pythagorean: `jiRatioWithState` in `tuning/ratios.ts` adds `db × (−15, +8, +1, 0)` to the prime-exponent vector (the schisma's prime decomposition) so the interval analyzer surfaces "octave + schisma" via existing comma machinery; `keyColorVariant` in `render/colors.ts` short-circuits to `computeHue(q, r)` directly, bypassing the SC-sibling redirect so each cell carries its actual 5-limit hue (matching the M3-chain respelling). **Naming caveat**: "Schismatic" here refers to the layout-level schisma stacking (every band carries one schisma; play across many bands and the schismas accumulate). It is NOT the classical schismatic temperament, which tempers the fifth itself by ~⅛ schisma so that 8 fifths approximate a pure 5/4. HKL's r-axis fifth stays pure 3:2 in V mode; only the band/q-axis accumulates schismas.
...
**Coloring**: SC-shifted cells (Pythagorean qm=1/qm=2, Semiditonal qm=2) reuse the colorTable via the SC equivalence — `lookupHue(q + 7·d, r − 4·d)` for −SC, `(−7, +4)` for +SC. An SC shift in (q,r) lands you on the cell whose hue is the SC sibling (purple → teal etc.). No new HueColors variants needed. Septimal qm=2 keeps its existing `.sl`/`.sd` warm-shifted variants. V mode is the lone exception — it skips the SC sibling redirect and colors every cell at its actual `lookupHue(q, r)`, matching the M3-chain respelling. Helper: `keyColorVariant(q, r)` in `src/render/colors.ts`.

**V-mode respelling** (`src/tuning/notes.ts:noteNameV` + `keyOctaveV`): V mode names cells by M3-distance from refSpine instead of via octave-invariant `noteName(q, r)`. Each Δq from refSpine applies one `m3up`/`m3dn` step; fifthName(r) anchors the fifth axis. Cells in the same band as refSpine spell identically to standard naming, but successive bands accumulate accidentals (A → G## → B##### at q+3, q+12 …). Accidentals are NOT clamped — beyond ±3 the cell labels render wider and Verovio (Composer) can't draw them cleanly. That's the documented experimental cost. The `displayedNoteName` helper in `render/draw.ts` and the resolveKey branch in `bridge/hkl-side.ts` route to `noteNameV` when `tuning.mode === 'V'`.
