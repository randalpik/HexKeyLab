...
const waitIdle = async (cdp) => {
  for (let i = 0; i < 400; i++) { if (await evalIn(cdp, `(() => { const b = document.getElementById('renderBusy'); return !!document.querySelector('.score-page svg') && (!b || b.hidden); })()`) === 'true') return; await sleep(250); }
};
try {
