...
apps/analyzer/
  ...
  cli/                    Node batch CLI (imports @hkl/analysis for k-weighting etc.)
    ...
    hki-regain.mjs            perceptual re-gain of a layered decay .hki (Bark-sones; see Gain normalization)
...
### Perceptual re-gain of layered decay bundles (`cli/hki-regain.mjs`)

Post-export pass for velocity-layered decay `.hki` files (HKLO output): rewrites only the per-sample `gain` (audio untouched) from a Bark-sones measurement (`@hkl/analysis/loudness.js`) over a 400 ms window starting at the engine's own perceptual onset (`findPerceptualOnset` from `@hkl/engine`). Motivation (2026-09-12 Korg SP-250 audit): the flat −18 dBFS normalization was peak-limited on 127/144 samples, so baked loudness followed crest factor (a 10.8 LUFS spread, ±5 dB per-note outliers, a C6 hole), and the brighter layers read as loudness tiers (+27 % sones at the 24→48 boundary at the same input velocity). Model: per note, every layer is targeted to the note's geometric-mean sones × tier^`--layer-span` (default 0 — Max's ruling: layers carry timbre, the house curve carries all loudness, so boundary crossings are continuous and loudness-vs-velocity is monotonic by construction); across notes, the reference follows a Gaussian-weighted local linear fit of log-sones vs MIDI (σ 6 semitones — removes note-to-note alternation, keeps the register curve) blended toward the set median by `--tilt` (default 0). Correction dB = `--strength` × 10·log₂(target/measured) (phon rule, as the loop-path `loudnessEvenness`), boosts allowed. **No level cap by default**: the −3 dBFS peak target is a normalization convention, not a limit — lowering the whole set to keep one boosted note under the master limiter would trade every note's RMS consistency for one transient; the report instead lists each layer's played peak at the top of its `pickLayer` zone. Calibration caveat: the lite model scales as gain^0.46 (≈13 dB/doubling), so re-measuring a corrected bundle shows ≈24 % of each deviation as residual; `--strength` (0..1.5) is one ear-trim knob. **Two endpoints, one slider** (2026-09-13): by ear the sones match over-corrected at the two lowest boundaries, so the bundle now carries both inter-layer matchings — `gain` (layers matched in Bark-sones) and `gainLevel` (layers matched in K-weighted level over the same 400 ms window; identical cross-note reference) — and the engine blends them log-linearly by its layer-blend setting (HKL lumadiag slider "Layer loudness match", 0 = level, 1 = sones, persisted as `prefs.layerBlend`). On the SP-250 the endpoints differ by −4.0 / −2.5 / −2.0 / −0.2 dB for the upper layer at the four boundaries. The slider is permanent (decisions.md "The layer-blend slider stays"), so a re-gained bundle always carries both gains; `--bake-blend <t>` collapses them to a single blended `gain` only for consumers that can't expose the blend (external `@hexkeylab/engine` embedders, `handoff/` bundles). Porting the model into HKLO's `buildHki` (replacing flat normalization + sweep softening) is the remaining step, and that port emits both gains too. → decisions.md "Perceptual re-gain of layered decay bundles".
