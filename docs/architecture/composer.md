...
- **`.musicxml` import** (`apps/composer/src/importMusicXml.ts`) — the inverse. Parses `<score-partwise>` → MEI string → `model.replaceDocument`. Forces **Equal / HEJI off / ignore-color on** (an imported score is the input to a later JI-retuning workflow). Maps: parts → instrument `<staffGrp>`s; `<staff>`+`<voice>` → global staff + 2 layers (Finale v1/2→staff1, v3/4→staff2); `<type>`+`<dot>` → `@dur`/`@dots` (divisions-independent; per-part `<divisions>` may differ); `<time-modification>`+`<tuplet>` → `<tuplet>`; `<tie>` → `@tie` intent; `<slur>` → `<slur startid/endid>`; staccato/accent/tenuto → `<artic>`; `<fermata>` → measure control event (appended **after** `<staff>`s — Verovio rejects pre-staff control events); `<direction>` dynamics → `<dynam>` (letter only; velocity via the doc's `hkl:dynamicMap`), `<wedge>` → `<hairpin>` (time-anchored `@tstamp`); pickup (`implicit="yes"`) → `hkl:pickup-ticks`; mid-piece key/meter → in-section `<scoreDef>`, clef → inline `<clef>`. Pitch→(q,r) preserves the source's exact enharmonic spelling — see `coordForSpelling` below. Measures are positional (`number="X4"`-style non-numeric ids are ignored).
