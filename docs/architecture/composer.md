...
## Engraving conventions on the render clone (2026-09-04)
...
- **`instrumentSpacing.ts`** — `staffDef@spacing="18"` (Verovio units) on the first staff of every instrument after the first: instruments sit 9 staff spaces apart while a grand staff's two staves keep Verovio's 6. Verovio's `spacingStaff` is a floor on EVERY adjacent pair and `spacingBraceGroup` only ever raises the inside of a brace, so the per-staff attribute is the only lever (probed). A minimum — content still pushes staves apart.
...
- **`sectionRestart.ts`** — `applySectionRestarts`: no courtesy key/meter signature before a section (movement) break. Each section-start measure's boundary `<scoreDef>` is wrapped in a content-less nested `<section restart="true">` placed before the break (Verovio's restart skips the standard cautionary and suppresses the key in its own), and a meter change gets `meter.form="invis"` — which blanks the courtesy meter AND the restart's own redraw — plus a visible layer-level `<meterSig>` in layer 1 of each staff of the section's first measure. Accepted Verovio side effects: the section's first system draws full instrument labels and is indented like the score's first system (a new movement restates its instruments), the blanked meter reserves about half a staff space before the visible one, and scroll view draws a clef at the restart measure. The document keeps its flat `scoreDef > sb > measure` shape — the wrapper holds no measure, so `querySelector('section')`, break pinning and the range clones are unaffected. → lessons.md "Verovio courtesy signatures". Fixture: `engr_sectionBreakNoCourtesySigs`.
...
- **Instrument gap** (2026-09-04) — 9 staff spaces between instruments against 6 inside a grand staff, via `staffDef@spacing` stamped on the render clone (`notation/instrumentSpacing.ts`); brace + `bar.thru` on grand staves only (see Bar lines + grand staff).
