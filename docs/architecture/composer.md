...
**Page-view system splice (Phase C-B v1, `apps/composer/src/render/pagesplice.ts`)**: when a refill's edit is provably local, the affected systems are re-engraved in a windowed offscreen render (pin-anchored, page geometry, `breaks:'line'`, with synthetic mRest leader + trailer lines absorbing the sub-document's score-start and end-of-score artifacts) and spliced into the mounted page SVGs — no full-doc `loadData` at all (~400–630 ms wall on the sonata vs 1.6–3.1 s full renders). The splicer is stateless (everything it needs lives in the DOM + the refill result) and gated hard: changed lines mounted (a placeholder page is mounted from the pre-edit layout, B5), spanner/`<ending>`-closed windows, unchanged neighbour lines must reproduce their live geometry and signature glyphs exactly (the divergence detector), and the window's own measured inter-system spacing gives every replaced system its position and the systems below it their common dy (B1) — anything the gates cannot prove falls back to the full refill render, loudly, with the reason on the splicer. Volta brackets ride inside `g.system` in page mode, so whole-system splices carry them for free. A signature-identical render request skips DOM work entirely (guards: interior scoreDef signature, credits in the head signature). No zone is excluded by NAME — line 0 and section-header lines were until 2026-09-01, and both proved to be stale guards rather than design (a header's title is re-placed from the `data-reserve`/`data-baseline` rule the mount-time injector records). Under `HKL_INDEX_CHECK` every splice is verified against an offscreen full render of the same pinned MEI — and that gate now actually fails the run: its throw used to be caught into a status-bar string by `doReRender`, which is fixed (always logs, re-throws under the check). Pagination is owned too (`<pb>` pins + `breaks:'encoded'`, C-B2a). B2 (2026-09-02) made the replaced set a line HUNK — N old systems give way to M new ones, each placed on the page the owned pagination assigns it — so line-count changes (a new final line while composing, a line whose measures were all deleted), moved page starts, page collapses and the overflow cascade (a page's spilled tail moves onto the next page as a page-first splice; a full last page spawns a new page from the window's own page SVG; an emptied page is removed and the rest renumbered) all land as splices. Page legality is overflow-only and measured live after surgery and after every mount; the cascade runs one page per step across the mounted set and settles lazily at mount time beyond it. The paper is fixed and the scale pinned at the box: a section header is a component with a reserved height in the page's vertical budget (the mount-time injector shifts its system and those below by the reserve and records it on the title), so a page that no longer fits below its headers spills into the same cascade, its title travelling with the moved system — D1's vertical distribution within that budget is the planned feature on top. See [docs/composer-page-splice-design.md](../composer-page-splice-design.md).
