...
- **Vertical placement** (2026-09-04): two mechanisms. Verovio's `dynamDist: 4.5` puts a dynamic's top 1.2 staff spaces below its staff (the option's effect only starts past 2 — probed glyph tops 1→40, 2→40, 3→72, 4→152 user units at unit 8, i.e. 80·d − 168; 3.5 read as "still a few px", Max 2026-09-05; it never governs `<dir>`). The DOM pass `render/textlayout.ts` (`layoutBelowStaffText`, run by `postProcessRendered` on every rendered system BEFORE placement measures it) then centres a `<dynam>`/`<hairpin>`/`<dir>` that sits between the two staves of a grand staff in the inter-staff gap, as far as the other staff's glyphs in its horizontal range allow, and under any other staff keeps a `<dir>` at the dynamics' clearance (`DIR_GAP_PER_UNIT`). Horizontally overlapping marks move as one block (a dynamic keeps its hairpin, a stacked dynamic + text keeps its order); reads and writes are two-phase, and every measurement is in the SVG's own user space (`getBBox` mapped through `getCTM`, whole-unit shifts) — never screen rectangles, which differ between the live page and the splice/reference hosts by their sub-pixel phase and flipped rule v2's quantization by a pixel on re-placed pages (gated sonata sweep, 2026-09-04); idempotent via `data-hkl-vshift`. Relies on `data-staff`/`data-place`/`data-n` from `svgAdditionalAttribute`. Not applied to the PDF (its pipeline never runs the DOM pass). Fixtures: `engr_dynamicCenteredInGrandStaff`, `engr_dirClearanceBelowSingleStaff`.
