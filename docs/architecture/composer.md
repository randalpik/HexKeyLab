...
**Page-view line-break ownership (Phase C-A, `apps/composer/src/render/linebreaks.ts`)**: in page view Composer — not Verovio's castoff — decides which measure starts each system. Derive renders (doc load, zoom, page-scale, staff-filter, any fallback) are byte-identical to the strategies above; the partition is then ADOPTED from the laid-out toolkit (idle-chunked, finished synchronously if an edit arrives first). An edit re-lays only the affected lines: the owner diffs the live doc against its own per-measure signature baseline (never the model's `renderDirty` hint — see lessons.md), measures natural widths for the changed window via an offscreen `breaks:'none'` sub-render (spanner/ending-complete), refills greedily (FIT_MAX 1.2 / MIN_FILL 0.7 + backward min-fill rebalance; user breaks are hard line starts; terminates on re-joining the old partition), and injects `<sb xml:id="hklpin-N">` pins into the render-time MEI only — the live doc, saves, undo and history never contain a pin. The pinned doc renders with `breaks:'line'` (every `<sb>` honored verbatim, pages still broken automatically by height — probed; `'smart'` can override pins, `'encoded'` doesn't paginate) or `'encoded'` for docs with user `<pb>`. Reflow semantics: the first edit in a region re-breaks it from the edited line by these rules; thereafter identical content always yields the identical partition and a no-change render moves nothing. Everything unprovable derives, loudly. This is the structural base for the Phase C system-splice (every boundary a hard anchor) — full design + status in [composer-page-splice-design.md](../composer-page-splice-design.md).
