...
- **`.musicxml` import** (`apps/composer/src/importMusicXml.ts`) — the inverse. Parses `<score-partwise>` → MEI string → `model.replaceDocument`. Forces **Equal / HEJI off / ignore-color on** (an imported score is the input to a later JI-retuning workflow). Maps: parts → instrument `<staffGrp>`s; `<staff>`+`<voice>` → global staff + 2 layers (Finale v1/2→staff1, v3/4→staff2); `<type>`+`<dot>` → `@dur`/`@dots` (divisions-independent; per-part `<divisions>` may differ); `<time-modification>`+`<tuplet>` → `<tuplet>`; `<tie>` → `@tie` intent; `<slur>` → `<slur startid/endid>`; staccato/accent/tenuto → `<artic>`; `<fermata>` → measure control event (appended **after** `<staff>`s — Verovio rejects pre-staff control events); `<ornaments><trill-mark>`/`<wavy-line>` → note-anchored `<trill>`; `<ornaments><tremolo>` → `<bTrem>` (single) or `<fTrem beams=N>` over the start/stop pair (fingered tremolo, each note drawn at the combined value — `ticks.ts` counts the pair as one note's duration); `<notehead>diamond` (**Finale's string-harmonic encoding — NOT `<technical><harmonic>`**) → `@head.shape="diamond"`+`@head.fill="void"`+`@data-hkl-harmonic`; `<direction>` dynamics → `<dynam>` (letter only; velocity via the doc's `hkl:dynamicMap`), `<wedge>` → `<hairpin>` (time-anchored `@tstamp`), non-tempo `<words>` → `<dir>` (italic from `font-style`; tempo captions excluded); `<rest measure="yes"/>` → a meter-filling rest (whole rest in 4/4, else beat-aligned via `decomposeBeatAlignedRests`); pickup (`implicit="yes"`) → `hkl:pickup-ticks`; mid-piece key/meter → in-section `<scoreDef>`; clefs → inline `<clef>` positioned by tick (scans **every** `<attributes>` block in the measure, so a true mid-measure clef change lands mid-layer, not at the bar head). **Beams**: the source `<beam>` grouping is reproduced by diffing it against our auto-beamer and stamping `@hkl-beam-break` only where they differ — the marker XORs against the natural beat boundary, so it expresses both mid-beat breaks AND cross-beat joins (see `naturalBeatGroupStarts` in `notation/beams.ts`). Pitch→(q,r) preserves the source's exact enharmonic spelling — see `coordForSpelling` below. Measures are positional (`number="X4"`-style non-numeric ids are ignored). **Not yet imported**: mid-piece tempo changes (only the first tempo caption is baked into the head `<tempo>`); secondary-beam hooks (Verovio derives them).
