...
- **`sectionRestart.ts`** — `blankSectionCourtesyMeters`: no courtesy METER before a section (movement) break. The boundary `<scoreDef>` gets `meter.form="invis"` (blanks every meter drawn from it, the courtesy included) and a visible layer-level `<meterSig>` is injected into layer 1 of each staff of the section's first measure, where it draws after clef and key. The courtesy KEY still draws: Verovio's only switch for it is `<section restart="true">`, which also restates the full instrument labels and indents the system like the score's first — a restart scoreDef with empty labels, bare staffDefs, or an empty `<labelAbbr>` in the head all still label (probed) — and Max ruled the restated names out (2026-09-05), so the restart wrapper that shipped on 09-04 was removed. → lessons.md "Verovio courtesy signatures". Fixture: `engr_sectionBreakNoCourtesySigs`.
