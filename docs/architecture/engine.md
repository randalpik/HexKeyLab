...
**Host AudioParam contract.** Because the `AudioContext` is host-injected, the engine must not assume spec-exact intrinsic-value semantics. On deferred-setter hosts (react-native-audio-api 0.13.2), `param.value =` is queued rather than applied, and evaluation at times before the first scheduled event resolves from the param's *constructor default* — never the intrinsic value. Engine convention (2.4.1): every `.value = x` write where `x` can differ from the param's default is paired with `param.setValueAtTime(x, 0)` — a t=0 timeline seed that is unconditionally the earliest event, making the timeline authoritative from birth on every host. Writes intended at the default (`voiceGain` etc. = 1.0) are exempt: there the wrong lookup returns the right value. Rationale + the on-device evidence: the canonical comment at `scheduleSegmentSwitch` and decisions.md "t=0 timeline seed".
...
**Instrument is per-voice, not global.** `sNoteOn(voiceKey, freq, velocity, instrumentKey, startAt?)` (and `sNoteOnFaded(..., instrumentKey, ...)`) take the instrument explicitly; each voice records it (`v.instr`), so `sNoteOff`/aftertouch/ramp resolve the *voice's* instrument. Multiple instruments therefore sound simultaneously with no mode-setting — the HKL host tracks its own `audio.activeWaveform` (what live input plays) and per-voice `activeOscs[vid].instr`, and passes the key in at each trigger. (There is no `setInstrument`/`isLoaded`; a former global-`currentInstrument` design required set-before-every-note — see decisions.md "instrument per-voice".)

**Voice identity is a `VoiceId`, not the lattice `KeyId`.** The engine's `voiceKey` (and the HKL host's `audio.activeOscs` / `sustainedKeys` keys) is a **`VoiceId`** (`apps/hkl/src/types.ts`): the bare `"q,r"` `KeyId` for live input and single-instrument playback, or a composite `instr\0q,r` for multi-instrument playback. This lets two DIFFERENT instruments sound the SAME `(q,r)` at once — they get distinct VoiceIds. The `KeyId` remains the visual/lattice identity (selection highlight, coloring), refcounted per coord during playback so a cell stays lit until every instrument's voice on it releases. Voices carry their own `q,r` so coord-needing ops read them off the voice rather than parsing the (now possibly prefixed) key. → see decisions.md "VoiceId decouples audio-voice identity from lattice KeyId".
