...
### 4.12a Piano output (external-synth JI playback)

The "Piano output" toolbar toggle (Piano tab, below "Piano input") mirrors HKL playback to an external MIDI synth at true just-intonation pitch, even on synths with **no pitch bend and no MPE** (e.g. Korg SP-250). Module: `src/midi/piano-out.ts`.

Mechanism — such synths honour **RPN 0001 (channel fine tuning)**, ±100¢ over 14 bits (center 8192, 81.92 steps/cent). A note-on re-applies the channel's *current* fine-tune to every voice on that channel, so distinct simultaneous tunings need distinct channels. We therefore allocate **one voice per MIDI channel** (`MpeAllocator`, constructor-configurable channel range/reuse/exclusions — default 2..16 LIFO for MPE export). Piano output uses **1..16 minus channels 10 and 16** in **FIFO** order: channel 10 is the General MIDI percussion channel (a note-on there plays a drum map, not a pitched note — verified by ear, it's what made notes "randomly" come out wrong/silent); channel 16 on this SP-250 ignores note-off (hung notes) and won't accept the RPN fine-tune (stuck at 0¢), so it's dead for melodic use. FIFO gives a reused channel's release tail time to decay before reuse. That leaves **14 melodic channels**. Per note-on: `keyFreq(q,r)` → nearest 12-TET note + cents (`freqToNoteFine`, A4=440 convention) → RPN 0001 fine-tune CCs, then note-on, on that channel.

Overflow — `syncPianoOut` is a membership diff, so a naive steal-oldest cascades: an evicted-but-still-wanted key looks "missing" next pass, gets re-started, evicts another, and storms the synth. Guard: keys evicted for exceeding the 14-voice cap go into a `shed` set (wanted but deliberately silent) and are skipped by the start loop until they leave `want` — newest 14 sound, the rest stay quiet, no churn. `restrikePianoOut` drops a key from `shed` so a deliberate re-strike can re-compete for a channel.

Hook — `syncPianoOut()` runs from `syncOutput()` (§5.3) tracking `selectedKeys ∪ sustainedKeys`. It self-gates on its own enabled flag and is **independent of `audioEnabled`** (unlike `syncAudio`, which short-circuits to `stopAllNotes()` when Audio is off), so muting Audio + enabling Piano output makes the external synth the sole sound source. Sustain needs no CC64 forwarding: HKL defers `engine.noteOff` until pedal release, so the synth sees note-off at the right time. Aftertouch is not forwarded (SP-250 doesn't receive it).

Pedal capture — a sustain pedal plugged into the piano-input device transmits CC 64 on the same port; `piano.ts`'s `pianoMessage` routes it (`handleSustainCC`) through the shared `pedal` state via `setDamperDepth`. It **always acts as sustain** (continuous half-damper — the SP-250 sends a stepped value), independent of the Lumatone toolbar's sustain/sostenuto `pedal.mode` dropdown (that's a Lumatone-side control; piano and Lumatone setups vary, so they're kept separate — add a dedicated piano-pedal control if a sostenuto option is later wanted). Because it feeds HKL's deferred note-off, held JI notes keep ringing on the external synth until release — important with Local Control off, where the SP-250's own pedal does nothing locally.

Rearticulation — re-striking a key held by the sustain pedal doesn't change `selected ∪ sustained` membership, so the diff alone wouldn't re-fire the note. `restrikePianoOut(key)` tears down the voice (note-off + free channel) at each re-strike site, placed before the `syncPianoOut()` that follows — so the sync re-acquires the channel and re-strikes (note-off → note-on) with the updated velocity. (The audio engine gets the same effect for free because the re-strike `noteOff` drops the voice from `activeOscs` and `syncAudio` re-creates it; piano-out has no `activeOscs` to diff against, hence the explicit call.)

Composer playback — the bridge dispatch (`bridge/hkl-side.ts`) drives the audio engine directly and calls `draw()` rather than `onSelectionChanged` (to avoid echoing held-keys back to Composer), so `syncOutput`/`syncPianoOut` doesn't run on its own. Piano output is wired in explicitly: `syncPianoOut()` is called at each point the dispatch mutates `selectedKeys` + `draw()`s (chord attack, per-voice note-off timer, and `abortActive`), and `restrikePianoOut(k)` runs in the back-to-back same-pitch branch. Slur glides (`glideVoices`, sustained instruments) surface to the synth as a note-off-old/note-on-new re-attack via the selection diff, since the SP-250 can't bend a held note.

Program mirroring — one-voice-per-channel means each of the 16 channels is an independent timbre slot on a multi-timbral synth, so a front-panel sound change (which the synth transmits as a Program Change, usually on ch1) leaves channels 2..16 on their default (piano). `pianoMessage` in `piano.ts` captures Program Change off the input port — regardless of the input-enabled gate — and calls `setOutputProgram()`, which logs the number (for building the program→instrument map by ear) and **broadcasts it to all 16 output channels**. Broadcast happens on PC capture and after `open()` resolves (re-asserting the program on rebind), never per-note (the CH345 adapter drops bytes under burst load). `currentProgram` is null until a PC is seen, so we never force a wrong default.

Channel levels — GM channels are independent "parts" with their own volume (CC7) and expression (CC11), so spreading voices across channels surfaced their differing defaults as note-to-note volume jumps. `broadcastChannelLevels()` forces every channel to a fixed level (CC7=100, CC11=127) on enable/rebind (after `open()`) and after each program change, alongside the program broadcast — outside note bursts.

Local control — so the raw 12-ET keys aren't heard while HKL drives the synth, `sendLocalControl(port, false)` sends Local Control Off (CC122=0, all channels) when output is enabled / a port is wired; CC122=127 restores it on disable, on switching ports, and on page close (`beforeunload`) so the keyboard is never left muted. CC122 isn't on the SP-250's published received-CC list, so it may be a no-op on some units (then Local Off is a panel setting).

Device — the output port **auto-matches the selected Piano input device by name** (`rebindPianoOut()`, called on enable, input-device change via `bindPort`, and MIDIAccess statechange); no separate device dropdown. The bound port is **explicitly `open()`ed** — inputs auto-wire their ALSA subscription when `onmidimessage` is set, but an output isn't routed to hardware until `open()`; Chrome opens eagerly on `send()`, **Firefox does not**, so without it `send()` drops silently (and an orphaned seq client from a prior tab can shadow the live one). Requires the synth set to receive on all channels (Omni On / multi-timbral). Persisted via `pianoOutputEnabled`.
...
### 4.15a Velocity architecture (canonical musical velocity)

`audio.keyVelocity[key]` holds a **canonical musical velocity (0–127)** — "how hard the player struck," normalized across input devices. Three clean stages:

1. **Input normalization → musical velocity (at input time, per device).**
   - Lumatone (`handler.ts`): `applyInputCurve(applyPerKeyGain(key, d2))` — per-key gain (hardware variance) then the Lumatone **target-gain curve** (`velocityCal.inputCurve`, floor/ceiling/gamma in gain space — the same shape the user tuned by ear pre-refactor). `applyInputCurve` maps raw velocity → target gain → **house-curve inverse** → musical velocity, so `curveGain(applyInputCurve(d2)) === targetGain(d2)` exactly: the Lumatone response is reproduced bit-for-bit while keyVelocity stays a device-neutral musical velocity. (A single velocity-space power law can't do this — the floor offset makes the exact inverse non-power-law; expressing it in gain space + inverting the house curve is exact.) Tuned in lumadiag's gain-space sliders (preview = the target curve). The firmware interval curve (CMD 0x20) is an orthogonal hardware-side alternative.
   - Piano / SP-250 (`piano.ts`): **identity** — a weighted MIDI keyboard already sends musical velocity.
   - QWERTY / mouse / Composer / recording: `DEFAULT_DYNAMIC_MAP` / score dynamics, in the musical domain.
2. **House curve (at audio time, device-independent):** `velocityCal.curveGain` (`velocityBaseVol`), gentle (floor 0.05 / ceiling 1.0 / **γ 1.5**), maps musical velocity → audio gain for samples and oscillators. No per-key prepass (that moved to input).
3. **External output:** `piano-out` sends `keyVelocity` unchanged, so an external synth applies only its own curve → velocity round-trips (play the SP-250, hear your dynamics back).

Why this shape: the old design folded the Lumatone's decompression into a steep γ≈15.5 audio curve and normalized every source into that "Lumatone domain" (piano input pre-inverted it). That broke the SP-250 round-trip (it re-applied its own curve on top) and squashed non-Lumatone sources. Splitting decompression (per-device input) from the gain curve (gentle, shared) makes velocity portable. `DEFAULT_DYNAMIC_MAP` was re-mapped from the old 96..127 cluster down to a musical spread (`v_new = houseCurve⁻¹(steepCurve(v_old))`) so QWERTY/mouse/Composer loudness is preserved, not amplified. `velocityCal` prefs carry `version: 3`; migration copies the old audio curve into the gain-space target curve verbatim (exact Lumatone reproduction) and resets the house curve to gentle (a short-lived v2 velocity-domain attempt is recovered by ×1.5 on its gamma). Aftertouch (`AFTERTOUCH_VEL_*`) and Composer hairpin swell (`HAIRPIN_OPEN_END_DELTA`) live in the musical domain now and may want by-ear tuning. `.hkr` is NOT migrated — pre-refactor recordings replay with shifted dynamics.
...
- `syncMidi()` — sends noteOn/noteOff in parallel (Lumatone output; selection only)
- `syncPianoOut()` — external-synth JI mirror (§4.12a); tracks selection ∪ sustained
...
- **`<slur>`** — *implemented*; note-attached via `@startid` / `@endid` (a slur's identity IS its two endpoint slots, so it stays note-attached, NOT in the tstamp expression layer). Lives in its own helper `src/composer/slurs.ts` (`addSlur` / `removeSlur` / `collectSlurs` / `pruneDanglingSlurs`). Endpoints are SLOT elements — a bare `<note>` or a `<chord>`, both id-bearing; the element is appended as a measure child after `<staff>` (same placement as `<hairpin>`/`<dynam>`) with a `data-voice` (1–4) recording its voice. **Entry (voice mode, `Ctrl+L`):** first press marks the start slot (pending state, status-line feedback only — no ghost arc); after navigating, a second `Ctrl+L` closes the slur (endpoints ordered low→high by flat-slot index; same slot → no-op + error). `Ctrl+L` on any slot **anywhere under an existing slur's span** deletes that whole slur. Switching voices (or entering select mode / Esc / undo) exits the pending state. Verovio renders the arc natively (no SVG injection); the element round-trips through `serialize` / `.hkc` untouched. **Dangling cleanup:** `pruneDanglingSlurs` runs inside `normalizeTies` (the shared post-mutation hook) and in `applyRetune` (which does its own tie cleanup); retune rewrites kept notes in place so their `xml:id`s — and thus their slurs — survive, only dropped endpoints prune. **Playback:** `buildPlayback` stamps each `PlaybackEvent` with `voice` and `slurredToNext` (true when the next attack in the same voice shares one slur span). Realization is instrument-dependent and therefore chosen HKL-side in `playScore` via `computeLegatoPlan(events, glideMode)`, with `glideMode = !instrReplaysOnTranspose()`:
  - **Decay + replay-on-transpose instruments** (overlap mode): a slurred note's release is delayed by `SLUR_OVERLAP_FRACTION` (12%) of its duration so its tail overlaps the next attack (note-proportional legato). Implemented as an `offMs` override on the off timer.
  - **Sustained loopers** (`replayOnTranspose:false`, glide mode — the "transpose effect"): a single-note slurred transition hands one voice off to the next pitch with a brief portamento (`SLUR_GLIDE_MS` = 70 ms, clamped to half the note) instead of re-attacking. The predecessor event sets `noOff` (its voice rides on); the successor sets `glideFromKey` and `dispatchChord` calls `glideVoices` (the shared batched voice-handoff primitive in `audio/engine.ts`, mirroring the live pitch-transpose: osc `frequency` ramp / sample `slideAndFadeOut` → `noteOnFaded`), migrating the playback bookkeeping (`heldKeys`, `voiceSeq`, `selectedKeys`/`playbackOwnedKeys`) old→new. Chord-involved slur joins fall back to normal abutting playback (v1). The glide is verified by ear, not the headless harness.
