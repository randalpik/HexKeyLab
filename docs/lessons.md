...
1. **predictNextRenderHeavy's duration memory (`lastFullMs`)**: one heavy fixture flips every SUBSEQUENT fixture's renders from synchronous to deferred (badge + double-rAF), changing screenshot timing against recorded baselines and breaking read-right-after-reRender assertions. The runner reset now clears it per fixture. **Update 2026-09-14 — the same trap fires WITHIN one fixture**, where the per-fixture reset cannot help: a fixture whose own setup derives (adding a section header, say) leaves `lastFullMs.page` near the 250 ms threshold, so whether its next `reRender()` is synchronous or deferred comes down to machine jitter on the preceding engrave — measured 221–298 ms on the same box, flipping `pageSystemSpliceCourtesyBehindSectionBreak` between pass and fail at 2-in-6. On the deferred path `reRender()` RETURNS BEFORE RENDERING, so the assertion samples the PREVIOUS render's `lastOutcome`/`lastDeriveReason` and reports a splice failure that never happened; worse, the still-queued render escapes the assertion and can be superseded by a full engrave, which then shows up as an `(unattributed)` full render in the splice ledger. Two defences, both shipped: the predictor no longer extrapolates the path (see decisions.md), and **`await window.__waitForRender()` after any `reRender()` whose outcome you then read** — it is free on the synchronous path, so there is no reason to omit it. Tell-tale: ledger length unchanged across the edit (no render happened at all), and failing runs costing ~one extra engrave more than passing ones.
...
## Verovio's restart cautionary runs at the restart scoreDef — a later scoreDef in the same run escapes it (2026-09-06)

`ScoreDefSetCurrentFunctor::VisitScoreDef` handles `<section restart>` by
running `SetCautionaryScoreDefFunctor(…, restart=true)` on the previous
measure IMMEDIATELY, with the upcoming scoreDef as it stands at that moment
(and it only clears the key). The system-break cautionary in `VisitMeasure`
is gated on `!m_restart`, and `m_restart` stays set until that measure is
visited. So a scoreDef that follows the restart scoreDef inside the same run
(a plain nested `<section>` — the labels trick already lives there) can carry
a CLEF change: `ReplaceDrawingValues` sets `DrawClef` on the upcoming
staffDef, no cautionary pass ever sees it, and the restart's `REDRAW_ALL`
draws it as the new system's clef. Put the clef in the restart scoreDef itself
and the restart's own cautionary pass draws it as a courtesy; leave it in the
layer and it draws after the barline behind the old system clef. Recipe in
`notation/sectionRestart.ts`; probed on the live 6.3.0 toolkit (sonata II→III,
zero console output).

## Verovio's autogenerated header steps the subtitle by the OUTER rend's line height (2026-09-06)

`PgHead::GenerateFromMEIHeader` puts the title (`x-large`, 540 px at the crisp
presets) and the subtitle (`small`, 288 px, after an `<lb/>`) in one
`<rend halign="center" valign="middle">`. The `<lb/>` advances the baseline by
the enclosing rend's line height — 347 user units on the sonata — which is less
than the title's own size, so the subtitle's text box (baseline − 0.9 × 288)
starts above the title's box bottom (baseline + 0.22 × 540): the two lines
overlap vertically and only clear horizontally when the title is short. No
option governs the header's line height; the fix is a DOM pass on the rendered
`g.pgHead` BEFORE placement (`render/pageheader.ts`), since the header's bbox
is what `firstContentTop` anchors the first system under.

## Verovio's two-voice rest offset is keyed to the other note's duration, not its ink (2026-09-06)

Sonata III, piano bass staff: m. 30's voice-1 eighth rest against a LOW half
note in voice 2 sat in the staff's upper half (loc 6); m. 35's identical rest
against a low QUARTER was lifted clear of the staff (bottom at the top line)
into its tuplet bracket, although the quarter's ink is no higher. Verovio's
`Rest::GetOptimalLayerLocation` looks the offset up by the other element's
duration, so "is there room on the staff" is not a question it asks. Pin
`@loc` from a prediction of the other layer's ink instead
(`notation/restlayout.ts`); `@loc` is honored inside tuplets too.

## `getBBox()` of a placed system omits its translate (2026-09-06)

Composer's placement moves every `g.system` with a `transform="translate"`.
`getBBox()` reports the box in the element's OWN frame, so an anchor computed
from a system's `bb.y` is wrong by exactly that translate — invisible while
page 1's first system sat where Verovio put it (translate ≈ 0), and 200 units
off the moment the title block grew and placement pushed the system down (the
composer credit stayed at the old height). Add `translateOf(el).ty`, or read
through `getCTM()` as `svgBox` does.

## A fixture clef via `setClefAt` needs the cursor ON the measure's first note; otherwise build the element (2026-09-06)

`m.setCursor(4, 1)` in a voice of whole notes resolved to the END of measure
3 (the clef was appended there — a courtesy-position clef, which the render
then drew identically to the old behaviour), and the flat index of measure 4's
note (5) resolved to AFTER that note. `setClefAtCursor` inserts before "the
content child at the cursor's tick", and a cursor index is not a
`flatChildren` index. For a layer-initial clef in a fixture, create the
`<clef>` element and `insertBefore(layer.firstChild)` — the shape the importer
and a correctly placed `setClefAt` both produce (`engr_sectionBreakNoCourtesyClef`).

## A notehead's ink box is 1.1 staff locations above and below its centre; Verovio's two-voice rest anchor is location 8 (2026-09-06)

Bare-toolkit measurements for the rest pass: a head centred on the middle
line (location 4) has its bbox top at 5.1, one at −1 tops out at 0.1 — the
glyph is 2.2 locations tall, not the 1.0 its nominal "one space" suggests, so
a clearance rule built on ±0.5 leaves a rest TOUCHING the head (Max's
regression: voice-1 eighth rest pinned at location 7 against a voice-2 chord
topping at the middle line). Verovio's own placements for reference: the
top-layer default anchor when the other layer has content is location 8 for
quarter AND eighth rests ([5, 11] and [6, 9.5] — three, resp. 1.5, locations
above the top line), a half rest goes to 8 (on the line), and its gaps to the
