...
| `↑` / `↓` | Switch voice / layer (and into the dynamics, pedal, and tempo layers — see below). Lands on the note sounding at the same moment, not the bar start. |
...
**Tuplets**: press `Ctrl+N` (where N is `2`–`7`) to start a tuplet, then a duration digit to set its base note. For example `Ctrl+3` then `5` is a triplet of eighth notes filling a quarter. A tuplet stays within one bar. (`Ctrl+R` is used for trills — see below — because Firefox reserves `Ctrl+T`.)
...
## Repeats, endings, octave lines & trills
...
A document starts as one piano (the grand staff). Add more instruments in **Setup → Instruments** (drag to reorder; changes apply on Save). Each instrument gets its own staves, voices, and dynamics/pedal layers.

- A toolbar **instrument selector** filters the view (and PDF export) to a single instrument's part.
- **Pizzicato / arco**: an expressive-text `pizz.`/`arco` cue switches the spanned notes to a pizzicato sample where one is available.
...
The **Setup…** button opens a dialog for:

- **Title** and **Composer** name.
- **Key signature** (with a major/minor toggle) and **time signature**.
- **Tempo** (♩ = …, with optional text).
- **Instruments** (add/remove/reorder — see above).
- **Dynamics → velocity** map (how loud each dynamic plays).
- **HEJI accidentals** and **Ignore color** toggles.
