...
/* Inject the footer (centered, in the bottom margin) into each rendered page
 * SVG and restyle the running header on pages 2+. The page-1 title block and
 * the composer credit are handled BEFORE placement by render/pageheader.ts
 * (`styleTitleBlock`, run from `postProcessRendered`) — the credit used to be
 * injected here, anchored to the first system, which put it beside the title
 * (2026-09-06). Coordinates: everything is appended into Verovio's
 * `g.page-margin` group; the usable column is read per page (`pageFrameOf`). */
...
const FOOTER_MARGIN_DROP = 370;
const HKL_SVG_NS = 'http://www.w3.org/2000/svg';

/* Running header on pages 2+ (2026-09-04). Verovio's autogenerated pgHead
 * draws "– N –" centred at the top. Convention instead: the page number goes
 * to the OUTER corner (even pages left, odd pages right — the edge away from
 * the binding) and the title runs centred on every page after the first.
 * Both keep the header's vertical band untouched: the number keeps its
 * baseline (so `g.pgHead`'s bbox height, which `firstContentTop` measures,
 * is unchanged) and the title is a sibling <text> in the page margin, not a
 * pgHead child. Page 1 keeps Verovio's title block. Idempotent — the position
 * is recomputed from the page's current number on every mount. */
function styleRunningHeader(pageMargin: Element, pageNo: number, title: string, frame: PageFrame): void {
  pageMargin.querySelector(':scope > text.hkl-running-title')?.remove();
  if (pageNo < 2) return;
  const head = pageMargin.querySelector(':scope > g.pgHead') as SVGGraphicsElement | null;
  const num = head?.querySelector('tspan.num') ?? null;
  const rend = num?.closest('tspan.rend') ?? null;
  if (!head || !num || !rend) return;
  /* Record the header's ORIGINAL ink bottom before restyling. Placement
     (`Renderer.headBottomOf`) measures the first system's clearance from it,
     and the restyled number reads one unit off Verovio's dashed original —
     enough to flip rule v2's quantization by a pixel on a re-placed page while
     the reference gate's fresh render did not move (2026-09-04). Same formula
     as headBottomOf: bbox bottom plus the group's own translate. */
  if (!head.hasAttribute('data-hkl-head-bottom')) {
    try {
      const bb = head.getBBox();
      if (bb.height > 0) head.setAttribute('data-hkl-head-bottom', String(bb.y + bb.height + translateOf(head).ty));
    } catch { /* not laid out yet — placement falls back to the live bbox */ }
  }
  ...
  rend.setAttribute('data-hkl-pgnum', String(pageNo));
  if (!title) return;
  const t = pageMargin.ownerDocument!.createElementNS(HKL_SVG_NS, 'text');
  t.setAttribute('class', 'hkl-running-title');
...
function injectHeaderFooter(scoreEl: HTMLElement, footer: string, title: string): void {
  ...
  for (const pageMargin of Array.from(margins)) {
    /* The running header keys on the page number; an offscreen host (no
       `.score-page` ancestor) is treated as page 1, matching the derive. */
    const ownerPage = pageMargin.closest('.score-page') as HTMLElement | null;
    const pageNo = ownerPage ? Number(ownerPage.dataset.page) : 1;
    const frame = pageFrameOf(pageMargin);
    styleRunningHeader(pageMargin, pageNo, title, frame);
    if (footer) {
      ...
      t.setAttribute('x', String(frame.innerW / 2));
      /* The drop into the margin scales with the margin itself. */
      t.setAttribute('y', String(frame.innerH + FOOTER_MARGIN_DROP * (frame.marginY / UNSCALED_MARGIN)));
      t.setAttribute('text-anchor', 'middle');
...
function injectSectionHeaders(scoreEl: HTMLElement, model: ComposerModel): void {
  ...
  for (const meas of headers) {
    ...
    /* Centered on the page (not the system — a short final section would
       otherwise pull the title to the left margin); the page's REAL column
       width, so a resized page centres it too. */
    t.setAttribute('x', String(pageFrameOf(pageMargin).innerW / 2));
...
async function bootRenderer(): Promise<void> {
  ...
  renderer.setOnPageMounted((pageEl) => {
    injectHeaderFooter(pageEl, model.getFooter(), model.getTitle());
...
$('btnSetup')?.addEventListener('click', () => {
  openSetupDialog(model, (layoutChanged) => {
    ...
    refreshViewSelector();
    /* Title, subtitle and composer live in Verovio's autogenerated header (and
       the pre-placement title-block pass): the refill / splice paths keep the
       old header, so a Setup apply re-engraves (Max, 2026-09-06: the Setup
       subtitle "doesn't actually do anything" — it only showed after a
       reload). Setup is a modal, not an editing keystroke; the cost is fine. */
    renderer.forceFullRerender();
    reRender();
    refreshIndicators();
