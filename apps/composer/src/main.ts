 ...
 * per-page facts — the constants above are the unscaled US-Letter case and
 * only a fallback. Until this, every injected text (footer, composer credit,
 * section titles) sat at the 100 % coordinates on a resized page: the footer
 * of a 140 % page landed a third of the way up the paper, off-centre. */
interface PageFrame { innerW: number; innerH: number; marginX: number; marginY: number }
const UNSCALED_MARGIN = 1400;
function pageFrameOf(pageMargin: Element): PageFrame {
...
function injectHeaderFooter(scoreEl: HTMLElement, composer: string, footer: string, title: string): void {
  ...
  for (const pageMargin of Array.from(margins)) {
    ...
    if (composer && pageNo === 1) {
      ...
      t.setAttribute('x', String(frame.innerW));
...
async function bootRenderer(): Promise<void> {
  ...
  renderer.setOnPageMounted((pageEl) => {
    injectHeaderFooter(pageEl, model.getComposer(), model.getFooter(), model.getTitle());
